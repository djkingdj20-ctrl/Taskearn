...
app.post('/api/login',(req,res)=>{
  const u=users.find(x=>
    x.email===req.body.email&&x.password===req.body.password
  );
...
app.get('/api/wallet',auth,(req,res)=>{
  ...
  res.json({
    ...
    withdrawals:withdrawals.filter(x=>x.userId===u.id)
  });
});
...
app.post('/api/tasks/:id/complete',auth,(req,res)=>{
  ...
  if(completed.has(key))
    return res.status(400).json({
      error:'Task already completed in this demo'
    });
...
/* Render / Production server configuration */

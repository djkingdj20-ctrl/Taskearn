...
async function sendEmailOtp(email, otp, purpose) {
  ...
  if (purpose === "login") {
    subject = "TaskEarn Login Verification Code";
  }

  if (purpose === "profile") {
    subject = "TaskEarn Mobile Verification";
  }
...
">
<h1 style="margin:0 0 10px;color:#171b2d;">
TaskEarn
</h1>
...
app.post("/api/auth/google", async (req, res) => {
  try {
    ...
    if (!user) {
      ...
      if (!user.profileImage && google.picture) {
    ...
    if (
      ...
      const step = await startVerification(
        req,
        user,
        "login"
      );
...
app.post("/api/register", async (req, res) => {
  try {
    ...
    const existingVerifiedMobile =
      findVerifiedUserByMobile(mobile);
    ...
    /*
      IMPORTANT:
      Email OTP is sent first.
      If email sending fails, the register
      request returns an immediate error.
    */
    ...
    if (step === "email") {
      return res.status(200).json({
        ...
        mobile: user.mobile,
        message:
          "Registration submitted successfully. Email OTP sent."
      });
    }
    ...
    if (step === "mobile") {
      return res.status(200).json({
        ...
        mobile: user.mobile,
        message:
          "Registration submitted successfully. Mobile OTP sent."
      });
    }
